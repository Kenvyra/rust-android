...
sed -i "s:1.69.0:1.71.0:g" toolchain/android_rust/paths.py

cd prebuilts/rust/linux-x86
mkdir 1.71.0
cd 1.71.0
curl https://ftp.travitia.xyz/clang/rust-1-71-0.tar.xz | tar -xJ

cd /build/rust-toolchain/toolchain/android_rust
git am -3 /build/0001-Fix-directory-creation.patch
cd /build/rust-toolchain

./toolchain/android_rust/fetch_source.py 1.72.0 --nightly

cp /build/rustc-0023-Add-Trusty-OS-support-to-Rust-std.patch toolchain/android_rust/patches/development/
cp /build/rustc-0022-PGO-staging.nightly.patch toolchain/android_rust/patches/development/rustc-0022-PGO-staging.patch
...
# Now update Rust's LLVM version to the latest bleeding edge
cd /build/rust-toolchain/toolchain/rustc/src
rm -rf llvm-project
git clone https://github.com/llvm/llvm-project --single-branch --branch main --depth 1
cd llvm-project
# This patch is required for rustc to build
git am -3 /build/0001-Add-accessors-for-MCSubtargetInfo-CPU-and-Feature-ta.patch
